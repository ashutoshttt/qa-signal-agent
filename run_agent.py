...
Pipeline:
  ...
  4. Enrich  — Apollo.io: company size check + QA contacts (score >= 7 only)
  5. Email   — HTML digest with highlights table + full job list
...
ENRICH_SCORE_THRESHOLD = 7   # only enrich companies scoring at or above this
...
def step_enrich(scored_jobs: list[dict]) -> list[dict]:
    logger.info("── Step 4/5: Apollo enrichment ─────────────────────")
...
def step_email(jobs: list[dict], dry_run: bool = False) -> None:
    logger.info("── Step 5/5: Email digest ──────────────────────────")
    ...
    # Merge Apollo enrichment from in-memory scored jobs into pending DB jobs
...
def main() -> None:
    ...
    step_email(enriched, dry_run=args.dry_run)
